...
shinyServer(function(input, output) {
  ...
  output$result <- renderText({
    ...
    name=substr(words, start=nchar(words), stop=nchar(words))
   ...
   if(length(as)>=1)
    {asf=strsplit(r$word[as[1]], " ")
      ...
      else if(nchar(words)==0 || substr(words, start=nchar(words), stop=nchar(words))==" ")
      {"Type the new word!"}
      else {result="We cannot predict next word"
      result}}
      else if(nchar(words)==0 || substr(words, start=nchar(words), stop=nchar(words))==" ")
      {"Type the new word!"}
       else {result="We cannot predict next word"
